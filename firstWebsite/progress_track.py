...
def progress_bar(request):
    try:
        ...
        form_wise_count = [1, non_teaching_instance, sum(item['count'] for item in faculty_participartion_data), sum(item['count'] for item in mooc_course_instance), sum(item['count'] for item in events_instance), sum(item['count'] for item in no_of_awards), sum(item['count'] for item in sponsored_research_instance), research_journal_instance, research_conference_instance, research_book_instance, patents_instance, sum(item['count'] for item in guided_instance), sum(item['count'] for item in resource_instance)]
